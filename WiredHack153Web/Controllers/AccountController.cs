...
namespace WiredHack153Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult AccountDisplay(int Id)
        {
            ...
            using (var db = new WiredHackEntities())
            {
                var itemUser = db.Users.Where(cc => cc.Id == Id);
                model.AccFirstName = itemUser.First().FirstName;
                model.AccLastName = itemUser.First().LastName;
                if (itemUser.First().EmployeeId.HasValue)
                {
                    var empId = itemUser.First().EmployeeId;
                    var itemEmployee = db.Employees.Where(cc => cc.Id == empId);
                    model.EmpAddress = itemEmployee.First().Address;
                }
                else
                {
                    model.EmpAddress = "Not Available";
                }
            }
            return View(model);
        }*/
